...
import WhiteFoxmodel from '../assets/white-fox.glb'

function WhiteFox() {
    const group = useRef<THREE.Group>(null)
    const { scene, animations } = useGLTF(WhiteFoxmodel)
    ...
    useFrame((_state, delta) => {
        ...
        target.current.y = -mouseY.current + 500
    ...
    return (
        <group ref={group} position={[7, -5, 0]}>
            <primitive
                object={scene}
                scale={6}
            />
